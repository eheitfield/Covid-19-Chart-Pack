...
```{r}
countryList <- c("US","GB","CN")

countryTable <- countries %>% 
  select(country,cases,deaths,todayCases,todayDeaths) %>%
  rename(Cases = cases, Deaths = deaths, "New Cases" = todayCases, "New Deaths" = todayDeaths) %>%
  arrange(desc(Cases)) %>%
  head(10)

knitr::kable( 
  countryTable,
  caption = "Top Countries by Cases", 
  digits = 8, format = "html",
...
usData %>% 
  ...
  knitr::kable(caption = "Last 7 Days", digits = c(8,7,7,7,7,2,2), format = "html", format.args = list(big.mark = ",")) %>% 
  kable_styling()
...
This section summarizes state-level data. State-level graphs are shown for the largest 15 states by population, which account for `r format(100*sum(largeStates$total_pop_15)/sum(states$total_pop_15), digits = 1)` percent of the total U.S. population.
...
topDeathsTable <- topDeaths %>%  
  rename( State = state,
          Cases = positive,
          Deaths = death)
knitr::kable(topDeathsTable, caption = "Top 10 States by Deaths", format = "html", format.args = list(big.mark = ",")) %>% 
  kable_styling()
```
...
topCasesTable <- topCases %>%
  rename(State = state,
         Cases = positive,
         Deaths = death)
knitr::kable(topCasesTable, caption = "Top 10 States by Cases", format = "html", format.args = list(big.mark = ",")) %>%
  kable_styling()
```
...
```{r}
ggplot(bigStatData, aes(x=date,y=weekChangeTests)) + 
  geom_col(alpha=0.5) + 
  geom_smooth(se=FALSE, span=0.5) + 
  labs(title = "7-Day Change in Daily Tests", x = "Date", y = "7-Day Change") +
  scale_y_continuous(labels = comma) +
  facet_wrap(~bigState, scales = "free_y") +
  scale_x_date(date_labels = "%b", breaks = dateBreaks)
```
...
```{r}
ggplot(bigStatData, aes(x=date,y=positiveTestRate)) +
  geom_line(alpha=0.4) +  
  geom_smooth(se=FALSE, span=0.5) + 
  labs(title = "Positive Test Rate by State", x = "Date", y = "Rate per Day") +
  scale_y_continuous(labels = percent, limits = c(0.0,0.4)) +
  facet_wrap(~bigState) +
  scale_x_date(date_labels = "%b", breaks = dateBreaks)
```
...
ggplot(stateWatchData, aes(x=wkPositiveTestRate,y=wkCasesPerCap,label=state)) + 
  geom_text(size = 2) +
  scale_x_continuous(limits = c(0.0,0.15), labels = percent) +
  labs(title = "New Cases vs. Positive Test Rate", x = "7-Day Positive Test Rate", y = "7-Day New Cases Per Million Pop.")
```
...
knitr::kable(recentLocal, caption = "Latest Local Data", format = "html", format.args = list(big.mark = ",")) %>%
  kable_styling()
```
